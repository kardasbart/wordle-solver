...
def print_filter_result(screen, result):
    stats, bestwords = calc_stats(result)
    screen.addstr(f"\n\nWords left: {len(bestwords)}\n\n")
    screen.addstr("Letters occurences according to filter:\n")
    for k, v in stats:
        screen.addstr(f'{k}: {v:.2f}%\n')

    screen.addstr("\nBest words:\n")
    for k,w in enumerate(bestwords[:5]):
        screen.addstr(f'{k}: \"{w[0]}\" score = {w[1]:.2f}\n')


def print_state(screen, hints, result):
    screen.clear()
    ...
    if hints is not None:
        ...
        if result is not None:
            print_filter_result(screen, result)
    else:
...
def main():
    ...
    while True:
        if current_hint is None:
            print_state(screen, current_hint, None)
            ...
            print_state(screen, current_hint, result)
            ...
            if func == 's':
                ...
                current_hint.correct(args[0], [int(x) for x in args[1:]])
            elif func == 'i':
                letter = args[0]
